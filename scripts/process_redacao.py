...
def gerar_relatorio_md(
    ...
    for comp, nota in detalhes.items():
        ...
        lines.append(f"### {comp} — {descricao} ({nota}/200)")
        lines.append("")
        # Gerar comentário baseado na faixa de nota
...
def processar_redacao(file_path: str, tema: str = "Tema Geral") -> None:
    ...
    # 3. Simulação da avaliação da IA por competências
    # Em produção, o texto seria enviado para uma LLM junto com o tema.
    # Aqui simulamos uma avaliação padrão se não houver anulação automática.
    if motivo_anulacao:
        notas_ia = {"C1": 0, "C2": 0, "C3": 0, "C4": 0, "C5": 0}
    else:
        print("[*] Simulando chamada da LLM para extração de pontos de interesse...")
        print("[*] Simulando chamada da LLM para avaliação baseada no contrato.yaml...")
        # Atribuição simulada de notas intermediárias
        notas_ia = {"C1": 120, "C2": 160, "C3": 120, "C4": 160, "C5": 120}
        logger.debug("LLM simulation completed")
        trace_data["steps"].append({"name": "llm_simulation", "status": "completed"})
